...
```bash
abc-renderer$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
abc-renderer$ sam local start-api
abc-renderer$ curl http://localhost:3000/
```
...
```bash
abc-renderer$ sam logs -n HelloWorldFunction --stack-name abc-renderer --tail
```
...
```bash
sam delete --stack-name abc-renderer
```
